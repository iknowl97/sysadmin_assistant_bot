...
from telegramAPItoken import telegram_api_token_for_sysadmin_assistant_Bot, telegram_api_token_for_web_panel_notif
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[
                        logging.FileHandler(f"{log_directory}/mainLog.txt"),
                        logging.FileHandler(
                            f"{log_directory}/connectionLost.txt"),
                        logging.FileHandler(
                            f"{log_directory}/connectionRestored.txt")
                    ])


def log_user_info(message):
    chat_user_client = message.from_user.username
    logging.info(f'ClientUserName: {chat_user_client}')


def log_message_to_file(message):
    chat_user_client = message.from_user.username if message.from_user.username else str(
        message.from_user.id)
    log_file_path = f"{log_directory}/{chat_user_client}.txt"
    log_entry = f"{datetime.now().strftime(
        '%Y-%m-%d %H:%M:%S')} - {message.text}\n"
